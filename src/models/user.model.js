...
class User {
  ...
  // Editar usuario
  static async updateUser(id, { name, surname, email, username, birthdate, img_user }) {
    const query = `
      UPDATE "user"
  ...
  static async getUserById(id) {
    const query = `SELECT *, TO_CHAR(birthdate, 'YYYY-MM-DD') AS birthdate FROM "user" WHERE id_user = $1`;
  ...
  static async findByEmailOrUsername(email, username) {
    const query = `SELECT *, TO_CHAR(birthdate, 'YYYY-MM-DD') AS birthdate FROM "user" WHERE email = $1 OR username = $2`;
